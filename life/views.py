...
def trombinoscope(request):
    personnes = Personne.objects.select_related('user')  # Récupère toutes les personnes
    data = []
    for personne in personnes:
        # Récupère le premier CV de la personne si disponible
        premier_cv = CV.objects.filter(personne=personne).first()
       
        Contacts =Contact.objects.filter(user=personne.user).first()
        data.append({
            'personne': personne,
...
def send_cv_email(request, cv_id):
    ...
    if request.method == 'POST':
        ...
        email = EmailMessage(
            subject=subject,
            body=message,
            from_email='kouakouclaudedoumy@gmail.com',
            to=[recipient_email],
        )
